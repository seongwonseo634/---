...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        data.push({ id: doc.id, ...doc.data() } as PersonalTransaction);
      });
  ...
  const dailyIncome = selectedDateTransactions.filter(t => t.type === 'income').reduce((acc, t) => acc + t.amount, 0);
  const dailyExpense = selectedDateTransactions.filter(t => t.type === 'expense').reduce((acc, t) => acc + t.amount, 0);
  ...
  return (
    <div className="p-4 space-y-6 max-w-3xl mx-auto h-full animate-in fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 neo-inset p-1 rounded-2xl">
          ...
          <span className="font-bold px-2">{format(currentDate, 'yyyy년 M월')}</span>
            ...
            {selectedDateTransactions.map(tx => (
              <div key={tx.id} className="neo rounded-2xl p-4 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 rounded-xl neo-inset flex items-center justify-center text-xl">
                    {tx.category === '식비' ? '🍔' : tx.category.includes('교통') ? '🚌' : tx.category.includes('쇼핑') ? '🛍️' : tx.category.includes('문화') ? '🎬' : '💳'}
                  </div>
                  <div>
